...
class NotificationService {
  ...
  static Future<void> scheduleTherapyNotification(
    ...
    await addScheduledTherapyNotification(
        id, title, body, time.toString(), category);
  }
  ...
  static Future<void> addScheduledTherapyNotification(
    ...
    therapyNotifications.add(
        '$id|$title|$body|$time|$category'); // Include category in the string
    await sharedPrefs.setStringList(
        'scheduled_therapy_notifications', therapyNotifications);
  }
  ...
  static Future<List<Map<String, String>>>
      getScheduledTherapyNotifications() async {
    List<String> therapyNotifications =
        sharedPrefs.getStringList('scheduled_therapy_notifications') ?? [];

    // Ensure the map only contains String values
    return therapyNotifications
        .map((notification) {
          List<String> parts = notification.split('|');
          ...
          if (parts.length == 5) {
            ...
            return <String,
                String>{}; // Return an empty map for invalid notifications
          }
        })
